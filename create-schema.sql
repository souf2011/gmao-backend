...
DROP TABLE IF EXISTS `gmao`.`adhesions`, `gmao`.`cache`, `gmao`.`cache_locks`, `gmao`.`etablissements`, `gmao`.`failed_jobs`, `gmao`.`job_batches`, `gmao`.`jobs`, `gmao`.`membres`, `gmao`.`migrations`, `gmao`.`password_reset_tokens`, `gmao`.`personal_access_tokens`, `gmao`.`roles`, `gmao`.`services`, `gmao`.`sessions`, `gmao`.`type_adhesions`, `gmao`.`users`;
...
INSERT INTO `roles` (
    ...
    updated_at
             ...
             1,                      -- role_id
             'ADMIN',               -- role (unique short name)
             'Administrateur',  -- role_label (human-readable)
             NOW(),                  -- created_at
             NOW()                   -- updated_at
         );
...
CREATE TABLE `services` (
                            ...
                            UNIQUE KEY `uniq_service_description` (`service_description`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
INSERT INTO `services` (
    ...
    updated_at
      ...
      (1, 'Welding', 'All types of welding services including MIG, TIG, and arc welding.', NOW(), NOW()),
      (2, 'Plumbing', 'Installation and repair of plumbing systems.', NOW(), NOW()),
      (3, 'Electrical', 'Electrical installations, repairs, and maintenance.', NOW(), NOW());
...
ALTER TABLE `users`
  ...
  ADD KEY `idx_service_id` (`service_id`),
   ADD KEY `idx_etablissement_id` (`etablissement_id`),
  ADD UNIQUE KEY `uniq_email` (`email`),
  ADD UNIQUE KEY `uniq_phone` (`phone`),
  ADD CONSTRAINT `fk_users_role` FOREIGN KEY (`role_id`) REFERENCES `roles` (`role_id`) ON UPDATE CASCADE ON DELETE RESTRICT,
    ADD CONSTRAINT `fk_users_service` FOREIGN KEY (`service_id`) REFERENCES `services` (`service_id`) ON UPDATE CASCADE ON DELETE RESTRICT,
    ADD CONSTRAINT `fk_users_etablissement` FOREIGN KEY (`etablissement_id`) REFERENCES `etablissements` (`id`) ON UPDATE CASCADE ON DELETE RESTRICT;
...
INSERT INTO `users` (
    ...
    updated_at
             ...
             '1985-07-15',           -- birthdate (YYYY-MM-DD)
             '+1234567890',          -- phone
             'john.doe@example.com', -- email
             NULL,                   -- email_verified_at
             'hashed_password_here', -- password (store hashed!)
             NULL,                   -- remember_token
             NULL,                   -- last_login
             NOW(),                  -- created_at
             NOW()                   -- updated_at
...
ALTER TABLE `adhesions`
  ...
  ADD CONSTRAINT `adhesions_user_id_foreign` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE;
